...
class ConcentrationPredictor:
    ...
    # 默认模型适用范围；加载到 pH 子模型后会优先使用子模型实际覆盖范围。
    VALID_PH_RANGE = (3.0, 8.0)
    ...
    def __init__(self, model_path: Optional[str] = None):
        ...
        self.ph_models = {}
        self.ph_stats = {}
        self.feature_cols = self.FEATURE_ORDER
        self.feature_cols_no_ph = self.FEATURE_ORDER_NO_PH
        self.version = "1.0"
        self._load_model()
        ...
        try:
            ...
            if isinstance(model_package, dict):
                ...
                if 'main_model' in model_package:
                    ...
                    self.feature_cols = model_package.get('feature_cols', self.FEATURE_ORDER)
                    self.feature_cols_no_ph = model_package.get('feature_cols_no_ph', self.FEATURE_ORDER_NO_PH)
    ...
    def _get_ph_model_key(self, ph: float) -> Optional[float]:
        """
        根据pH值确定使用哪个子模型。
        
        子模型按训练集中出现的 pH 值建立。连续 pH 会根据相邻
        子模型 pH 的中点边界路由到最近的子模型；例如支持 pH=7
        和 pH=8 时，pH=7.5 会路由到 pH=8 子模型。
        """
        if not self.ph_models:
            return None

        keys = sorted(float(k) for k in self.ph_models.keys())
        if len(keys) == 1:
            return keys[0]

        boundaries = [(keys[i] + keys[i + 1]) / 2.0 for i in range(len(keys) - 1)]
        for key, upper_boundary in zip(keys, boundaries):
            if ph < upper_boundary:
                return key
        return keys[-1]

    def _get_supported_ph_range(self) -> Tuple[float, float]:
        """返回当前模型实际覆盖的 pH 范围。"""
        if self.ph_models:
            keys = sorted(float(k) for k in self.ph_models.keys())
            return keys[0], keys[-1]
        return self.VALID_PH_RANGE
    ...
    def predict(self, feature_vector: np.ndarray, use_ph_model: bool = True) -> Dict[str, Any]:
        ...
        model_feature_vector = np.array(
            [feature_dict[col] for col in self.feature_cols]
        ).reshape(1, -1)
        ph_model_feature_vector = np.array(
            [feature_dict[col] for col in self.feature_cols_no_ph]
        ).reshape(1, -1)
        ...
        if use_ph_model and ph_model_key is not None and ph_model_key in self.ph_models:
            # 使用子模型预测
            prediction = self.ph_models[ph_model_key].predict(ph_model_feature_vector)[0]
            method = f'ph_{ph_model_key}_model'
            logger.info(f"使用 pH={ph_model_key} 子模型预测: {prediction:.4f} mM (输入pH={ph:.1f})")
        else:
            # 使用主模型预测
            prediction = self.main_model.predict(model_feature_vector)[0]
            method = 'main_model'
            ph_model_key = None
            logger.info(f"使用主模型预测: {prediction:.4f} mM (输入pH={ph:.1f})")
        ...
        logger.info(f"最终预测结果: {prediction:.4f} mM (方法: {method})")
        
        return result
    
    def _calculate_confidence(self, prediction: float, ph: float, ph_model_key: Optional[float]) -> float:
        """
        计算置信度
    ...
    def _generate_warnings(self, features: np.ndarray, prediction: float, ph_model_key: Optional[float]) -> List[str]:
        """生成使用警告"""
        warnings = []
        ...
        supported_ph_min, supported_ph_max = self._get_supported_ph_range()

        # pH 范围检查
        if ph < supported_ph_min or ph > supported_ph_max:
            warnings.append(
                f"pH 值 {ph:.1f} 超出当前模型训练覆盖范围 "
                f"({supported_ph_min:g}-{supported_ph_max:g})，"
                f"预测结果可能不可靠"
            )
        
        # 检查是否使用了子模型（仅当存在子模型时显示此警告）
        if ph_model_key is None and len(self.ph_models) > 0:
            if supported_ph_min <= ph <= supported_ph_max:
                warnings.append(
        ...
        if prediction < 0:
            warnings.append(
                f"预测浓度为负值 ({prediction:.4f} mM)，"
                f"可能是输入图像或 pH 值有误"
            )
        elif prediction > 20:
            warnings.append(
                f"预测浓度 {prediction:.4f} mM 超出典型范围，"
                f"请检查输入数据"
            )
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            'ph_model_features': self.feature_cols_no_ph,
            'valid_ph_range': self._get_supported_ph_range(),
